"""
modernbert_weight.py — Class-Weighted ModernBERT Classifier (answerdotai/ModernBERT-base)
-----------------------------------------------------------------------------------------
Architecture:
  - Pretrained answerdotai/ModernBERT-base via Hugging Face transformers
  - Fully fine-tuned sequence classification model (all layers trainable)
  - Native long-context encoder architecture evaluated at seq len 2048
...
Workflow:
  ...
  - Save all per-class metrics for test and yt in results/modernbert_weight_metrics.json and models/weighted_modernbert.json
"""
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune Class-Weighted answerdotai/ModernBERT-base on dataset_split")
    ...
    print("\n" + "═" * 70)
    print("HINGLISH SCAM CALL CLASSIFIER — CLASS-WEIGHTED MODERNBERT-BASE FINE-TUNING")
    print(f"Model: {args.model_name}")
    print(f"Device: {DEVICE} | Max Seq Len: {args.max_seq_len} | Data root: {data_root}")
    print("═" * 70)
    ...
    ckpt_path = cache_dir / "modernbert_weight_best.pt"
    ...
    hf_save_dir = cache_dir / "modernbert_weight_base_best"
    ...
    single_model_json = {
        "model_name": "ModernBERT_Base_Weighted",
    ...
    for od in out_dirs:
        ...
        res_file = od / "modernbert_weight_metrics.json"
        with open(res_file, "w", encoding="utf-8") as f:
            json.dump(single_model_json, f, indent=2)
        print(f"💾 Saved Weighted ModernBERT Base metrics → {res_file}")

    # Also save weighted_modernbert.json in models/
    with open("models/weighted_modernbert.json", "w", encoding="utf-8") as f:
        json.dump(single_model_json, f, indent=2)
    print("💾 Saved Weighted ModernBERT metrics → models/weighted_modernbert.json")
