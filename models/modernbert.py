"""
modernbert.py — Hinglish Scam Call Classifier (answerdotai/ModernBERT-base)
-------------------------------------------------------------------------
Architecture:
...
Workflow:
  ...
  - Save all per-class metrics for test and yt in results/modernbert_metrics.json
"""
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune answerdotai/ModernBERT-base on dataset_split")
    ...
    print("\n" + "═" * 70)
    print("HINGLISH SCAM CALL CLASSIFIER — MODERNBERT-BASE FINE-TUNING")
    ...
    ckpt_path = cache_dir / "modernbert_best.pt"
    ...
    hf_save_dir = cache_dir / "modernbert_base_best"
    ...
    for od in out_dirs:
        ...
        res_file = od / "modernbert_metrics.json"
        with open(res_file, "w", encoding="utf-8") as f:
            json.dump(single_model_json, f, indent=2)
        print(f"💾 Saved ModernBERT Base metrics → {res_file}")
