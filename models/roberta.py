"""
roberta.py — Hinglish Scam Call Classifier (FacebookAI/roberta-base)
-------------------------------------------------------------------
Architecture:
...
Workflow:
  ...
  - Save all per-class metrics for test and yt in results/roberta_metrics.json
"""
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune FacebookAI/roberta-base on dataset_split")
    ...
    print("\n" + "═" * 70)
    print("HINGLISH SCAM CALL CLASSIFIER — ROBERTA-BASE FINE-TUNING")
    ...
    ckpt_path = cache_dir / "roberta_best.pt"
    ...
    hf_save_dir = cache_dir / "roberta_base_best"
    ...
    for od in out_dirs:
        ...
        res_file = od / "roberta_metrics.json"
        with open(res_file, "w", encoding="utf-8") as f:
            json.dump(single_model_json, f, indent=2)
        print(f"💾 Saved RoBERTa Base metrics → {res_file}")
