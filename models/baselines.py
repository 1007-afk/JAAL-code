"""
baselines.py — TF-IDF Baselines for Hinglish Scam Classifier
-----------------------------------------------------------
Models:
...
Workflow:
  ...
  - Save all per-class and overall metrics for test and yt in a single JSON file.
"""
...
def compute_detailed_metrics(gold, preds, model_name: str, split_name: str):
    ...
    target_names = [ID2NAME[i] for i in range(len(LABEL_MAP))]
    ...
    cm = confusion_matrix(gold, preds, labels=list(range(len(LABEL_MAP))))
...
def main():
    parser = argparse.ArgumentParser(description="TF-IDF Baselines on dataset_split")
    ...
    print("\n" + "═" * 70)
    print("HINGLISH SCAM CLASSIFIER — TF-IDF BASELINES")
    ...
    # 2. Fit TF-IDF on Train set only
    ...
    for model_name, clf in models.items():
        print("\n" + "─" * 70)
        print(f"Training {model_name} on Train Set...")
        print("─" * 70)
        t_start = time.time()
        ...
        # Combine into single JSON for this model
        single_model_json = {
    ...
    # Also save combined baselines_metrics.json
    for od in out_dirs:
        combined_path = od / "baselines_metrics.json"
        with open(combined_path, "w", encoding="utf-8") as f:
            json.dump(all_models_metrics, f, indent=2)
        print(f"\n💾 Saved all baseline metrics → {combined_path}")

    # Summary table in console
    print("\n" + "═" * 85)
    print("FINAL SUMMARY: TEST SET vs REAL YT DATASET")
    print("═" * 85)
    print(f"{'Model':<22} | {'Test Acc':>9} {'Test MacF1':>11} | {'YT Acc':>9} {'YT MacF1':>11}")
    print("-" * 85)
