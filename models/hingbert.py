"""
hingbert.py — Hinglish Scam Call Classifier (l3cube-pune/hing-bert)
-------------------------------------------------------------------
Architecture:
  - Pretrained l3cube-pune/hing-bert (a BERT model pretrained on Hinglish text) via Hugging Face transformers
  - Fully fine-tuned sequence classification model (all layers trainable)
  - MPS (Mac Apple Silicon), CUDA, and CPU acceleration
...
Workflow:
  ...
  - Save all per-class metrics for test and yt in results/hingbert_metrics.json
"""
...
MAX_SEQ_LEN = 512  # BERT-base has 512 position embeddings: 512 is the maximum usable length
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune l3cube-pune/hing-bert on dataset_split")
    ...
    print("\n" + "═" * 70)
    print("HINGLISH SCAM CALL CLASSIFIER — HINGBERT FINE-TUNING")
    ...
    # 4. Model instantiation & full fine-tuning
    ...
    print(f"\n🚀 Fine-tuning for up to {args.epochs} epochs (Early stopping patience={EARLY_STOPPING_PATIENCE})...\n")
    best_acc, best_state = 0.0, None
    epochs_no_improve = 0

    for epoch in range(1, args.epochs + 1):
        t0 = time.time()
        tr_loss, tr_acc = train_epoch(model, tr_loader, optimizer, scheduler)
    ...
    ckpt_path = cache_dir / "hingbert_best.pt"
    ...
    hf_save_dir = cache_dir / "hingbert_base_best"
    ...
    np.savez(Path(args.output_dir) / "hingbert_predictions.npz", te_pred=np.array(te_preds), te_gold=np.array(te_gold), yt_pred=np.array(yt_preds), yt_gold=np.array(yt_gold))
    ...
    for od in out_dirs:
        ...
        res_file = od / "hingbert_metrics.json"
        with open(res_file, "w", encoding="utf-8") as f:
            json.dump(single_model_json, f, indent=2)
        print(f"💾 Saved HingBERT metrics → {res_file}")
