"""
encoder.py — Hinglish Scam Call Classifier (Transformer Encoder)
---------------------------------------------------------------
Architecture:
...
def main():
    parser = argparse.ArgumentParser(description="Train & Evaluate Transformer Encoder on dataset_split")
    ...
    print("\n" + "═" * 70)
    print("HINGLISH SCAM CALL CLASSIFIER — TRANSFORMER ENCODER (~1M Params)")
    ...
    # 2. Tokenizer trained on Train split only
    cache_dir = Path("models/cache")
    cache_dir.mkdir(parents=True, exist_ok=True)
    tok_cache = cache_dir / "encoder_bpe_tokenizer.json"
    if tok_cache.exists():
    ...
    ckpt_path = cache_dir / "encoder_best.pt"
    ...
    for od in out_dirs:
        ...
        res_file = od / "encoder_metrics.json"
        with open(res_file, "w", encoding="utf-8") as f:
            json.dump(single_model_json, f, indent=2)
        print(f"💾 Saved Transformer Encoder metrics → {res_file}")
